...
exports.update = function (req, res, next) {
	db.query("UPDATE degree SET total_units='"+req.body.units+"' description='"+req.body.name+"' degree_code='"+req.body.code+"' WHERE degree_id='"+req.params.id+"'", function (err, rows) {
		if(err) return(err);
		if(rows.length === 0)
			res.send(404, {message: 'Degree program not found.'});
		else res.send(rows);
